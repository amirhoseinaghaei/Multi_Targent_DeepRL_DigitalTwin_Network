...
class PolicyGradientAgent(object):
    ...
    def choose_action(self, observation):
        ...
        # if t < 100e3: 
        #     return T.tensor(0.0, requires_grad=True).item()
        if observation[2] <= SimulationParams.NumberOfCpuCycles[0]:
            return 0.0
       
        probabilities = F.softmax(self.policy.forward(observation))  # Calculate softmax
        action_probs = T.distributions.Categorical(probabilities)
        action = action_probs.sample()
        log_probs = action_probs.log_prob(action)
        self.action_memory.append(log_probs)
        
        # Scale action to be between 0 and 1 (in increments of 0.1)
        scaled_action = action.item() / (self.policy.n_actions - 1)
