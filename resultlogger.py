...
# data2 = json.load(f)
f = open("./pytorch_models/5ps_140_10_withpower_nonsimple_0.01/Results.json")
data3 = json.load(f)
f = open("./pytorch_models/5ps_140_10_withpower_nonsimple_0.01/Results2.json")
data4 = json.load(f)
...
for i in range(1,NumberOfPS+1):
    ...
    plt.plot(range(500, (len(data3[f"{i}"])+1)*500,500), data3[f"{i}"], label = f"PS{i}", linestyle = "-.")
    plt.xlabel("Episode")
    plt.ylabel('Average AoI')
# plt.axhline(y = 80, color = "orange", linestyle = 'solid', label = "boundry", )
...
for i in range(1,NumberOfPS+1):
    ...
    plt.plot(range(500, (len(data4[f"{i}"])+1)*500,500), data4[f"{i}"] ,label = f"PS{i}", linestyle = "-.")
    plt.xlabel("Episode")
