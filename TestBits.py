...
Test = True
Results = {"5ps":{"simple" : {"AoI":[], "Power":[]}, "nonsimple_0.01": {"AoI":[], "Power":[]}}}
bits = ["120", "125", "130", "135", "140", "145"]
  ...
  for bit in bits:
    for rew in reward_mode:
      for ch in channelList:
        ...
        policy[1].load(f"{1}th PS", f"./pytorch_models/bits/5ps_{bit}_{ch}_withpower_{rew}")
        policy[2].load(f"{2}th PS", f"./pytorch_models/bits/5ps_{bit}_{ch}_withpower_{rew}")
        policy[3].load(f"{3}th PS", f"./pytorch_models/bits/5ps_{bit}_{ch}_withpower_{rew}")
        policy[4].load(f"{4}th PS", f"./pytorch_models/bits/5ps_{bit}_{ch}_withpower_{rew}")
        policy[5].load(f"{5}th PS", f"./pytorch_models/bits/5ps_{bit}_{ch}_withpower_{rew}")
        ...
        Results["5ps"][rew]["AoI"].append(mean/5)
        Results["5ps"][rew]["Power"].append(average_power)
...
#   plt.plot(channelList, Results["140"]["simple"]["AoI"], label = "Average AoI Violation of all PSs 120 bits- fixed αᵢ = 1",  linestyle = "dashed", marker = "*")
  plt.plot(bits, Results["5ps"]["nonsimple_0.01"]["AoI"] , label = "AoI Violation of all PSs 120 bits - αᵢ using equation 21",  linestyle = "solid", marker = "*")
  ...
  plt.xlabel('Nₖ')
  plt.xticks(bits, labels=[f'{x}' for x in bits])
  ...
  plt.plot(bits, Results["5ps"]["nonsimple_0.01"]["Power"] , label = "Average Power Usage of all PSs 120 bits - αᵢ using equation 21",  linestyle = "solid", marker = "*")
...
#   plt.plot(channelList, Results["5PS"]["simple"]["Power"], label = "Average Power Usage of all PSs 120 bits - fixed αᵢ = 1",  linestyle = "dashed", marker = "*")
