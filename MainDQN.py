  ...
  for j in range(eval_episodes):
    ...
    while time < 200:
      ...
      for i in range(1,NumberOfPS+1):
        ...
        action[i] = policy[i].choose_action(states[i], True)
...
for i in range(1,NumberOfPS+1):
  ...
  policy[i] =  Agent(gamma=0.99, epsilon=1.0, lr=1e-4,
                  input_dims=[SimulationParams.NumberOfTCh + 2], n_actions=5, mem_size=100000, eps_min=0.01,
...
while total_timesteps < max_timesteps:
    ...
    for ps in range(1, NumberOfPS + 1):
        ...
        actions[ps] = policy[ps].choose_action(states[ps], False)
    ...
    for ps in range(1, NumberOfPS + 1):
        ...
        policy[ps].store_transition(states[ps], actions[ps], rewards[ps], next_states[ps], terminal[ps])
        policy[ps].learn()
