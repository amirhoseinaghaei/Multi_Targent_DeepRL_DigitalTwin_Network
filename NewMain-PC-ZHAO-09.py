...
if Test == True:
  ...
  plt.axhline(y = 30, color = "orange", linestyle = 'solid', label = "deadline ps2", )
  plt.legend(loc = "best")
  ...
  plt.axhline(y = 30, color = "orange", linestyle = 'solid', label = "deadline ps3", )
  plt.legend(loc = "best")
    ...
    for ps in range(1,NumberOfPS+1):
      if episode_timesteps[ps] >= 200:
        if total_timesteps != 0:
          ...
          central_critic = policy[ps].train(central_critic, episode_timesteps[ps], policy, replay_buffer, ps, batch_size, discount, tau, noise_clip, policy_freq)
